...
export function DayCard({
  ...
  return (
    <div
      className={cn(
        "flex flex-col rounded-xl border transition-all duration-300 min-h-[200px]",
        isToday && "ring-1 ring-[#00d4ff]/40",
        isMatch &&
          "border-[#a855f7]/40 shadow-[0_0_15px_rgba(168,85,247,0.15)]",
        !isMatch && "border-white/[0.08]",
        session
          ? "glass glass-hover"
          ...
          <div className="space-y-2.5">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-sm font-medium capitalize text-white/90">
                  {session.type === "training" ? "Training" : session.type}
                </span>
              </div>
            ...
            {session.restPlayers.length > 0 && (
              <div className="flex items-start gap-1.5 text-xs text-[#ff6b35]">
                ...
                <span>
                  Rest: {session.restPlayers.map((p) => `#${p.jerseyNumber}`).join(", ")}
                </span>
              </div>
            )}
            ...
            {expanded && session.notes && (
              <div className="text-xs text-white/40 italic border-t border-white/[0.04] pt-2 mt-1">
                {session.notes}
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col items-center justify-center h-full gap-3 py-4">
            ...
            {onAddSession && (
              <button
                ...
                className="flex items-center gap-1 text-xs text-[#00d4ff]/60 hover:text-[#00d4ff] hover:bg-[#00d4ff]/10 px-2.5 py-1.5 rounded-lg transition-all duration-200 border border-[#00d4ff]/20 hover:border-[#00d4ff]/40"
              >
